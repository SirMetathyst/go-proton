...
func EntityIndexAttribute_C_1_7_0(m *entitas.M) string {
	b := new(bytes.Buffer)
	if m.EntityIndex() == entitas.SingleEntityIndex {
		b.WriteString("Entitas.CodeGeneration.Attributes.PrimaryEntityIndex")
	} else if m.EntityIndex() == entitas.MultipleEntityIndex {
...
func CleanupModeAttribute_C_1_7_0(cp *entitas.CP) string {
	b := new(bytes.Buffer)
	if cp.IsCleanup() {
...
func EventAttribute_C_1_7_0(cp *entitas.CP) string {
	b := new(bytes.Buffer)
	if cp.IsEvent() {
...
func UniqueAttribute_C_1_7_0(cp *entitas.CP) string {
	b := new(bytes.Buffer)
	if cp.IsUnique() {
...
func FlagPrefixAttribute_C_1_7_0(cp *entitas.CP) string {
	b := new(bytes.Buffer)
	if cp.HasFlagPrefix() {
...
func ContextAttribute_C_1_7_0(cl []*entitas.C) string {
	b := new(bytes.Buffer)
	for i, c := range cl {
...
func ComponentAttribute_C_1_7_0(cp *entitas.CP) string {
	slice := make([]string, 0)
...
func MemberAttribute_C_1_7_0(m *entitas.M) string {
	slice := make([]string, 0)
	out := new(bytes.Buffer)
